...
def role_required(allowed_roles):
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if 'PATIENT' in allowed_roles and hasattr(request.user, 'patient_profile'):
                has_permission = True
            elif 'DENTISTE' in allowed_roles and hasattr(request.user, 'dentiste_profile'):
                has_permission = True
            elif 'SECRETAIRE' in allowed_roles and hasattr(request.user, 'secretaire_profile'):
                has_permission = True
